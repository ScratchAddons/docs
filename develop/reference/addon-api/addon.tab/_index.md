...
addon.tab.addEventListener("urlChange", function (event) {
  console.log(
    `URL changed! It was previously ${event.detail.oldUrl}, it's now ${event.detail.newUrl}`
  );
});
```
...
**In most cases, you should use the [`userscripts` property of the addon manifest](https://scratchaddons.com/develop/reference/addon-manifest/#userscripts-and-userstyles-array) instead.**
